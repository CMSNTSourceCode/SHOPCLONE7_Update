...
if($_POST["action"] == "removeAccountSold") {
    ...
    if($isRemove) {
        $Mobile_Detect = new Mobile_Detect();
        $CMSNT->insert("logs", ["user_id" => $getUser["id"], "ip" => myip(), "device" => $Mobile_Detect->getUserAgent(), "createdate" => gettime(), "action" => "Xóa tài khoản (" . $row["uid"] . ") khỏi đơn hàng đã bán"]);
...
if($_POST["action"] == "removeSupplier") {
    ...
    if($supplier["status"] == 1) {
        exit(json_encode(["status" => "error", "msg" => "Vui lòng tắt trạng thái API trước khi xóa"]));
    }
...
if($_POST["action"] == "removeAccountStock") {
    ...
    if($isRemove) {
        $Mobile_Detect = new Mobile_Detect();
        $CMSNT->insert("logs", ["user_id" => $getUser["id"], "ip" => myip(), "device" => $Mobile_Detect->getUserAgent(), "createdate" => gettime(), "action" => "Xóa tài khoản (" . $row["uid"] . ") khỏi kho hàng đang bán"]);
        $data = json_encode(["status" => "success", "msg" => "Xóa tài khoản " . $row["uid"] . " thành công"]);
        exit($data);
    }
}
