...
if(isset($_POST["action"])) {
    ...
    if($_POST["action"] == "Login") {
        ...
        $CMSNT->update("users", ["remember_token" => $remember_token, "token_2fa" => NULL, "limit_2fa" => 0, "token_otp_mail" => NULL, "limit_otp_mail" => 0, "otp_mail" => NULL, "ip" => myip(), "time_request" => time(), "time_session" => time(), "update_date" => gettime(), "device" => $Mobile_Detect->getUserAgent()], " `id` = '" . $getUser["id"] . "' ");
    ...
    if($_POST["action"] == "Verify2FA") {
        ...
        $remember_token = generateRememberToken($getUser["remember_token"], $getUser["ip"]);
        $CMSNT->update("users", ["remember_token" => $remember_token, "token_2fa" => NULL, "limit_2fa" => 0, "ip" => myip(), "time_request" => time(), "time_session" => time(), "update_date" => gettime(), "device" => $Mobile_Detect->getUserAgent()], " `id` = '" . $getUser["id"] . "' ");
        if($getUser["status_noti_login_to_mail"] == 1 && $CMSNT->site("email_temp_subject_warning_login") != "") {
            $content = $CMSNT->site("email_temp_content_warning_login");
            $content = str_replace("{domain}", $_SERVER["SERVER_NAME"], $content);
            $content = str_replace("{title}", $CMSNT->site("title"), $content);
    ...
    if($_POST["action"] == "VerifyOTP") {
        ...
        $remember_token = generateRememberToken($getUser["remember_token"], $getUser["ip"]);
        $CMSNT->update("users", ["remember_token" => $remember_token, "token_otp_mail" => NULL, "limit_otp_mail" => 0, "otp_mail" => NULL, "ip" => myip(), "time_request" => time(), "time_session" => time(), "update_date" => gettime(), "device" => $Mobile_Detect->getUserAgent()], " `id` = '" . $getUser["id"] . "' ");
        if($getUser["status_noti_login_to_mail"] == 1 && $CMSNT->site("email_temp_subject_warning_login") != "") {
            $content = $CMSNT->site("email_temp_content_warning_login");
            $content = str_replace("{domain}", $_SERVER["SERVER_NAME"], $content);
            $content = str_replace("{title}", $CMSNT->site("title"), $content);
    ...
    if($_POST["action"] == "ChangePasswordProfile") {
        ...
        $token = generateUltraSecureToken(64);
        $isUpdate = $CMSNT->update("users", ["remember_token" => bin2hex(random_bytes(64)), "password" => TypePassword(check_string($_POST["newpassword"])), "token" => $token], " `token` = '" . check_string($_POST["token"]) . "' ");
        if($isUpdate) {
    ...
    if($_POST["action"] == "ChangePassword") {
        ...
        $token = generateUltraSecureToken(64);
        $isUpdate = $CMSNT->update("users", ["remember_token" => bin2hex(random_bytes(64)), "token_forgot_password" => NULL, "password" => TypePassword($password), "token" => $token], " `id` = '" . $getUser["id"] . "' ");
        if($isUpdate) {
