...
if($_REQUEST["action"] == "buyProduct") {
    ...
    if(!($product = $CMSNT->get_row("SELECT * FROM `products` WHERE `id` = '" . check_string((int) $_REQUEST["id"]) . "' AND `status` = 1 "))) {
    ...
    if($getUser["discount"] == 0) {
        ...
        if(!empty($_REQUEST["coupon"])) {
            $discount_coupon = checkCoupon($product["id"], check_string($_REQUEST["coupon"]), $getUser["id"], $money);
        }
        ...
        if($product["supplier_id"] != 0) {
            if($supplier["type"] == "SHOPCLONE6") {
                ...
                if(!isset($data)) {
                    ...
                    exit(json_encode(["status" => "error", "msg" => __("Mất kết nối đến kho hàng")]));
                }
            ...
            if($supplier["type"] == "API_26") {
                ...
                if(!isset($data["status"]) || $data["status"] != "ok") {
                    $User->RefundCredits($getUser["id"], $pay, "[Error] " . __("Hoàn tiền đơn hàng mua tài khoản") . " <b>" . $product["name"] . "</b> - #" . $trans_id, "REFUND_" . $trans_id);
                    exit(json_encode(["status" => "error", "msg" => __($data["error"])]));
                }
                ...
                foreach (explode("\n", $response) as $account) {
                    if($account == "") {
                        ...
                        $uid = explode(" ", $account);
                        $isInsertAPI = $CMSNT->insert("product_sold", ["type" => $supplier["domain"], "product_code" => NULL, "supplier_id" => $product["supplier_id"], "trans_id" => $trans_id, "buyer" => $getUser["id"], "seller" => $product["user_id"], "uid" => $uid[0], "account" => str_replace(":", "|", $account), "create_gettime" => gettime()]);
                        if($isInsertAPI) {
                            $isValue++;
                        }
                    }
            ...
            try {
                ...
                $User->RefundCredits($getUser["id"], $pay, __("[Error 1] Hoàn tiền đơn hàng mua tài khoản do lỗi hệ thống") . " #" . $trans_id, "REFUND_" . $trans_id);
                exit(json_encode(["status" => "error", "msg" => __("Có lỗi xảy ra, vui lòng thử lại sau")]));
            }
        }
        if(0 < $isValue) {
            if($product["supplier_id"] == 0 && $isValue < $amount) {
                ...
                $User->RefundCredits($getUser["id"], $pay, "[Error 2] Hoàn tiền đơn hàng mua tài khoản #" . $trans_id, "REFUND_" . $trans_id);
                exit(json_encode(["status" => "error", "msg" => __("Số lượng còn lại trong hệ thống không đủ")]));
            } else {
                $commission_fee = 0;
    ...
    if($_REQUEST["action"] == "total_payment") {
        ...
        exit(json_encode(["status" => "success", "money" => format_currency($money), "discount" => format_currency($money - $pay), "discount_number" => $money - $pay, "pay" => format_currency($pay)]));
    }
