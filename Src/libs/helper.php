...
function checkBlockIP($type, $time = 15)
{
    ...
    if($type == "API") {
        ...
        $max_attempts = $CMSNT->site("limit_block_ip_api");
    } elseif($type == "LOGIN") {
        ...
        $max_attempts = $CMSNT->site("limit_block_ip_reset_password");
    } elseif($type == "OTP") {
        $reason = "Spam OTP";
        $max_attempts = $CMSNT->site("limit_block_ip_otp");
    } elseif($type == "2FA") {
        $reason = "Spam 2FA";
        $max_attempts = $CMSNT->site("limit_block_ip_2fa");
    } elseif($type == "PAYMENT") {
        $reason = "Spam Tạo hóa đơn nạp tiền quá nhiều lần";
        $max_attempts = $CMSNT->site("limit_block_ip_payment");
    } else {
        $reason = "Spam Request quá nhiều lần";
        $max_attempts = $CMSNT->site("limit_block_ip_spam");
    }
    $CMSNT->insert("failed_attempts", ["ip_address" => $ip_address, "attempts" => 1, "type" => $type, "create_gettime" => gettime()]);
    ...
    if($max_attempts <= $attempts["total"]) {
        $CMSNT->insert("block_ip", ["ip" => $ip_address, "attempts" => $attempts["total"], "create_gettime" => gettime(), "banned" => 1, "reason" => __($reason)]);
...
function checkDomainAPI($domain, $proxy = "")
{
    ...
    if(!$checkdomain["status"]) {
        return ["msg" => $checkdomain["msg"], "status" => false];
    }
...
function check_img($img)
{
    ...
    $ext = end($ext);
    $valid_ext = ["png", "jpeg", "jpg", "PNG", "JPEG", "JPG", "gif", "GIF", "svg", "SVG", "webp", "WEBP"];
...
function checkAddonLicense($licensekey, $project)
{
    ...
    if(function_exists("curl_exec")) {
        ...
        if($fp) {
            ...
            while (!@feof($fp) && $status) {
                $line = @fgets($fp, 1024);
                if(!$responseCode && preg_match($responseCodePattern, trim($line), $m)) {
    ...
    if($responseCode != 200) {
        return ["status" => false, "msg" => "Remote Check Failed (HTTP " . $responseCode . ")"];
    }
    ...
    if(isset($results["md5hash"]) && $results["md5hash"] != md5($licensing_secret_key . $check_token)) {
        return ["status" => false, "msg" => "MD5 Checksum Verification Failed"];
    }
    $status = $results["status"] ?? "Invalid";
    $status_messages = ["Active" => ["Kích hoạt giấy phép thành công!", true], "Invalid" => ["Giấy phép kích hoạt không hợp lệ", false], "Expired" => ["Giấy phép mã nguồn đã hết hạn, vui lòng gia hạn ngay", false], "Suspended" => ["Giấy phép của bạn đã bị tạm ngưng", false], "timeout" => ["Yêu cầu kiểm tra giấy phép đã hết thời gian chờ", true]];
    if(isset($status_messages[$status])) {
        list($msg, $stt) = $status_messages[$status];
        return ["msg" => $msg, "status" => $stt];
    }
    return ["msg" => "", "status" => true];
}
