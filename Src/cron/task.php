...
foreach ($CMSNT->get_list(" SELECT * FROM `automations` ") as $task) {
        ...
        if($task["product_id"] == "") {
            ...
            foreach (json_decode($task["product_id"], true) as $product) {
                if($product_code = $CMSNT->get_row(" SELECT * FROM `products` WHERE `id` = '" . $product . "' ")["code"]) {
                    $isUpdate = $CMSNT->update("product_stock", ["product_code" => $task["other"], "create_gettime" => gettime()], " `product_code` = '" . $product_code . "' AND " . time() . " - UNIX_TIMESTAMP(create_gettime) >= " . $task["schedule"] . " ");
                }
            }
        }
    }
    ...
    if($task["type"] == "delete_order_not_uid") {
        if($task["product_id"] == "") {
            $CMSNT->update("product_order", ["trash" => 1], " " . time() . " - UNIX_TIMESTAMP(create_gettime) >= " . $task["schedule"] . " ");
            $isUpdate = $CMSNT->update("product_sold", ["account" => __("Tài khoản đã được xóa tự động")], " " . time() . " - UNIX_TIMESTAMP(create_gettime) >= " . $task["schedule"] . " ");
        } else {
            foreach (json_decode($task["product_id"], true) as $product) {
                foreach ($CMSNT->get_list(" SELECT * FROM `product_order` WHERE `product_id` = '" . $product . "' AND " . time() . " - UNIX_TIMESTAMP(create_gettime) >= " . $task["schedule"] . " AND `trash` = 0 ") as $product_order) {
                    $CMSNT->update("product_order", ["trash" => 1], " `trans_id` = '" . $product_order["trans_id"] . "' ");
                    $isUpdate = $CMSNT->update("product_sold", ["account" => __("Tài khoản đã được xóa tự động")], " `trans_id` = '" . $product_order["trans_id"] . "' ");
                }
            }
        }
    }
    if($task["type"] == "delete_order_revenue") {
        if($task["product_id"] == "") {
            ...
            foreach (json_decode($task["product_id"], true) as $product) {
                foreach ($CMSNT->get_list(" SELECT * FROM `product_order` WHERE `product_id` = '" . $product . "' AND " . time() . " - UNIX_TIMESTAMP(create_gettime) >= " . $task["schedule"] . " ") as $product_order) {
