...
$CMSNT = new DB();
if($CMSNT->site("time_cron_suppliers_api25") < time() && time() - $CMSNT->site("time_cron_suppliers_api25") < 2) {
    exit("Thao tác quá nhanh, vui lòng thử lại sau!");
}
$CMSNT->update("settings", ["value" => time()], " `name` = 'time_cron_suppliers_api25' ");
foreach ($CMSNT->get_list(" SELECT * FROM `suppliers` WHERE `status` = 1 AND `type` = 'API_25' ") as $supplier) {
