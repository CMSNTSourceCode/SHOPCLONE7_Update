...
$CMSNT = new DB();
if($CMSNT->site("time_cron_suppliers_api26") < time() && time() - $CMSNT->site("time_cron_suppliers_api26") < 2) {
    exit("Thao tác quá nhanh, vui lòng thử lại sau!");
}
$CMSNT->update("settings", ["value" => time()], " `name` = 'time_cron_suppliers_api26' ");
foreach ($CMSNT->get_list(" SELECT * FROM `suppliers` WHERE `status` = 1 AND `type` = 'API_26' ") as $supplier) {
    ...
    if(isset($result["goods"])) {
        foreach ($result["goods"] as $product_id => $api) {
            ...
            $api_stock = (int) check_string($api["count"]);
            $api_desc = NULL;
            $api_price = (int) check_string($api["price_wmz"]);
            $ck = $api_price * $supplier["discount"] / 100;
            $price = (int) check_string($api["price_wmz"]);
