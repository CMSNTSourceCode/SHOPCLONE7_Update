...
if($CMSNT->site("task_24h") < time() && 86400 < time() - $CMSNT->site("task_24h")) {
    $CMSNT->update("settings", ["value" => time()], " `name` = 'task_24h' ");
    $isRemove = $CMSNT->remove("failed_attempts", " `create_gettime` <= NOW() - INTERVAL 1 DAY ");
    if($isRemove) {
        $CMSNT->insert("logs", ["user_id" => 0, "action" => "Hệ thống thực hiện dọn dẹp failed_attempts sau mỗi 24 giờ", "createdate" => gettime(), "ip" => myip(), "device" => $Mobile_Detect->getUserAgent()]);
    }
    if(is_dir(__DIR__ . "/../CMSNT.CO")) {
        deleteFolder(__DIR__ . "/../CMSNT.CO");
        $CMSNT->insert("logs", ["user_id" => 0, "action" => "Hệ thống thực hiện xóa file rác", "createdate" => gettime(), "ip" => myip(), "device" => $Mobile_Detect->getUserAgent()]);
    }
}
if($CMSNT->site("status_tao_gd_ao") == 1) {
    ...
    if($int_rand == $CMSNT->site("toc_do_gd_nap_ao")) {
        ...
        $CMSNT->insert("deposit_log", ["user_id" => $CMSNT->get_row("SELECT * FROM `users` ORDER BY RAND() ")["id"], "method" => $method, "amount" => (double) $amount, "received" => (double) $amount, "create_time" => time(), "is_virtual" => 1]);
    }
