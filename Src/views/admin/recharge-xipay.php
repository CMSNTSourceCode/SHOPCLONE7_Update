...
$body = ["title" => __("Recharge XiPay"), "desc" => "CMSNT Panel", "keyword" => "cmsnt, CMSNT, cmsnt.co,"];
...
if(isset($_POST["SaveSettings"])) {
    ...
    $CMSNT->insert("logs", ["user_id" => $getUser["id"], "ip" => myip(), "device" => $Mobile_Detect->getUserAgent(), "createdate" => gettime(), "action" => __("Cấu hình nạp tiền XiPay")]);
    ...
    $currentYear = date("Y");
    $currentDate = date("Y-m-d");
    $total_yesterday = (int) $CMSNT->get_row("SELECT SUM(price) FROM payment_xipay WHERE `status` = 1 AND  `created_at` LIKE '%" . $yesterday . "%' ")["SUM(price)"];
    ...
    if(!$checkKey["status"]) {
        ...
        echo $CMSNT->site("gateway_xipay_status") == 1 ? "selected" : "";
        echo "                                                    value=\"1\">ON\n                                                </option>\n                                                <option\n                                                    ";
        echo $CMSNT->site("gateway_xipay_status") == 0 ? "selected" : "";
        ...
        echo $CMSNT->site("gateway_xipay_license");
        ...
        echo $CMSNT->site("gateway_xipay_rate");
    ...
    echo format_currency($CMSNT->get_row("SELECT SUM(price) FROM payment_xipay WHERE `status` = 1 AND MONTH(created_at) = '" . $currentMonth . "' AND YEAR(created_at) = '" . $currentYear . "' ")["SUM(price)"]);
    echo "                                        </p>\n                                        <p class=\"mb-0 text-muted\">Tháng ";
    echo date("m");
    echo "</p>\n                                    </div>\n                                    <div class=\"ms-2\">\n                                        <span class=\"avatar text-bg-info rounded-circle fs-20\"><i\n                                                class='bx bxs-wallet-alt'></i></span>\n                                    </div>\n                                </div>\n                            </div>\n                        </div>\n                    </div>\n                    <div class=\"col-xl-6\">\n                        <div class=\"card custom-card\">\n                            <div class=\"card-body\">\n                                <div class=\"d-flex align-items-center\">\n                                    <div class=\"flex-fill\">\n                                        <p class=\"mb-1 fs-5 fw-semibold text-default\">\n                                            ";
    echo format_currency($CMSNT->get_row("SELECT SUM(price) FROM payment_xipay WHERE `status` = 1 AND YEAR(created_at) = " . $currentYear . " AND WEEK(created_at, 1) = " . $currentWeek . " ")["SUM(price)"]);
    ...
    echo __("Số lượng (￥)");
    echo "</th>\n                                        <th class=\"text-center\">";
    echo __("Số tiền nhận");
    echo "</th>\n                                        <th class=\"text-center\">";
    echo __("Phương thức");
    echo "</th>\n                                        <th class=\"text-center\">";
    echo __("Thời gian tạo");
    echo "</th>\n                                        <th class=\"text-center\">";
    echo __("Thời gian cập nhật");
    echo "</th>\n                                    </tr>\n                                </thead>\n                                <tbody>\n                                    ";
    ...
    echo __("Paid:");
    echo "                                                <strong\n                                                    style=\"color:red;\">";
    echo format_currency($CMSNT->get_row(" SELECT SUM(`price`) FROM `payment_xipay` WHERE " . $where . " ")["SUM(`price`)"]);
