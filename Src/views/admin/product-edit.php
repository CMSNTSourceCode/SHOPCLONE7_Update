...
if(isset($_POST["save"])) {
    ...
    $isInsert = $CMSNT->update("products", ["stt" => !empty($_POST["stt"]) ? check_string($_POST["stt"]) : 0, "code" => !empty($_POST["code"]) ? check_string($_POST["code"]) : NULL, "name" => !empty($_POST["name"]) ? check_string($_POST["name"]) : NULL, "slug" => !empty($_POST["slug"]) ? check_string($_POST["slug"]) : NULL, "images" => trim($images), "short_desc" => !empty($_POST["short_desc"]) ? check_string($_POST["short_desc"]) : NULL, "description" => !empty($_POST["description"]) ? base64_encode($_POST["description"]) : NULL, "flag" => !empty($_POST["flag"]) ? check_string($_POST["flag"]) : NULL, "note" => !empty($_POST["note"]) ? base64_encode($_POST["note"]) : NULL, "text_txt" => !empty($_POST["text_txt"]) ? check_string($_POST["text_txt"]) : NULL, "price" => !empty($_POST["price"]) ? check_string($_POST["price"]) : 0, "min" => !empty($_POST["min"]) ? check_string($_POST["min"]) : 1, "max" => !empty($_POST["max"]) ? check_string($_POST["max"]) : 100000, "sold" => !empty($_POST["sold"]) ? check_string($_POST["sold"]) : 0, "cost" => !empty($_POST["cost"]) ? check_string($_POST["cost"]) : 0, "discount" => !empty($_POST["discount"]) ? check_string($_POST["discount"]) : 0, "check_live" => !empty($_POST["check_live"]) ? check_string($_POST["check_live"]) : "None", "category_id" => check_string($_POST["category_id"]), "status" => check_string($_POST["status"]), "order_by" => !empty($_POST["order_by"]) ? check_string($_POST["order_by"]) : 1, "update_gettime" => gettime(), "allow_api" => !empty($_POST["allow_api"]) ? check_string($_POST["allow_api"]) : 0], " `id` = '" . $product["id"] . "' ");
    if($isInsert) {
...
echo "            ";
if($product["check_live"] == "Instagram" && 120 <= time() - $CMSNT->site("time_cron_checklive_instagram")) {
    echo "            <div class=\"alert alert-danger alert-dismissible fade show custom-alert-icon shadow-sm\" role=\"alert\">\n                <svg class=\"svg-danger\" xmlns=\"http://www.w3.org/2000/svg\" height=\"1.5rem\" viewBox=\"0 0 24 24\"\n                    width=\"1.5rem\" fill=\"#000000\">\n                    <path d=\"M0 0h24v24H0z\" fill=\"none\" />\n                    <path\n                        d=\"M15.73 3H8.27L3 8.27v7.46L8.27 21h7.46L21 15.73V8.27L15.73 3zM12 17.3c-.72 0-1.3-.58-1.3-1.3 0-.72.58-1.3 1.3-1.3.72 0 1.3.58 1.3 1.3 0 .72-.58 1.3-1.3 1.3zm1-4.3h-2V7h2v6z\" />\n                </svg>\n                Vui lòng thực hiện <b><a target=\"_blank\" class=\"text-primary\"\n                        href=\"https://help.cmsnt.co/huong-dan/huong-dan-xu-ly-khi-website-bao-loi-cron/\">CRON\n                        JOB</a></b> liên kết: <a class=\"text-primary\"\n                    href=\"";
    echo base_url("cron/checklive/instagram.php");
    echo "\"\n                    target=\"_blank\">";
    echo base_url("cron/checklive/instagram.php");
    echo "</a> 1 phút 1 lần để hệ\n                thống\n                check live tài khoản.\n                <button type=\"button\" class=\"btn-close\" data-bs-dismiss=\"alert\" aria-label=\"Close\"><i\n                        class=\"bi bi-x\"></i></button>\n            </div>\n            ";
}
...
echo __("Số thứ tự:");
echo "                                        <span class=\"text-danger\">*</span></label>\n                                    <input type=\"number\" class=\"form-control\" name=\"stt\" value=\"";
echo $product["stt"];
echo "\"\n                                        required>\n                                </div>\n                                <div class=\"col-sm-12 mb-2\">\n                                    <label class=\"form-label\" for=\"example-hf-email\">";
...
echo __("Số lượng đã bán: (nếu có)");
...
echo "                                            value=\"Hotmail\">Hotmail & Outlook (check bằng imap)\n                                        </option>\n                                        <option ";
echo $product["check_live"] == "Gmail" ? "selected" : "";
echo " value=\"Gmail\">\n                                            Gmail (Cấu hình API Key tại Cài đặt -> Kết nối)\n                                        </option>\n                                        <option ";
echo $product["check_live"] == "Instagram" ? "selected" : "";
echo "                                            value=\"Instagram\">Instagram (Cấu hình API Key tại Cài đặt -> Kết nối)\n                                        </option>\n                                    </select>\n                                </div>\n\n                                <div class=\"col-sm-4 mb-2\">\n                                    <label class=\"form-label\" for=\"example-hf-email\">";
echo __("Trạng thái:");
...
echo base_url("ajaxs/admin/create.php");
...
echo __("Ngẫu nhiên");
echo "</option>\n                                    </select>\n                                </div>\n                                <div class=\"col-sm-12 mb-2\">\n                                    <label class=\"form-label\"\n                                        for=\"example-hf-email\">";
echo __("Cho phép kết nối API");
echo "</label>\n                                    <select class=\"form-control\" name=\"allow_api\" required>\n                                        <option ";
echo $product["allow_api"] == 1 ? "selected" : "";
echo " value=\"1\">\n                                            ";
echo __("ON");
echo "</option>\n                                        <option ";
echo $product["allow_api"] == 0 ? "selected" : "";
echo " value=\"0\">\n                                            ";
echo __("OFF");
