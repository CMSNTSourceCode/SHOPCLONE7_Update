...
$body = ["title" => __("Recharge Bakong Wallet Cambodia"), "desc" => "CMSNT Panel", "keyword" => "cmsnt, CMSNT, cmsnt.co,"];
...
if(isset($_POST["SaveSettings"])) {
    ...
    $CMSNT->insert("logs", ["user_id" => $getUser["id"], "ip" => myip(), "device" => $Mobile_Detect->getUserAgent(), "createdate" => gettime(), "action" => __("Cấu hình nạp tiền Bakong Wallet Cambodia")]);
    ...
    $currentYear = date("Y");
    $currentDate = date("Y-m-d");
    $total_yesterday = (int) $CMSNT->get_row("SELECT SUM(price) FROM payment_bakong WHERE `status` = 1 AND  `created_at` LIKE '%" . $yesterday . "%' ")["SUM(price)"];
    ...
    if(!$checkKey["status"]) {
        ...
        echo $CMSNT->site("bakong_status") == 1 ? "selected" : "";
        echo "                                                    value=\"1\">ON\n                                                </option>\n                                                <option ";
        echo $CMSNT->site("bakong_status") == 0 ? "selected" : "";
        ...
        echo $CMSNT->site("bakong_profile_id");
        ...
        echo $CMSNT->site("bakong_profile_key");
        ...
        echo $CMSNT->site("bakong_rate");
        echo "\" name=\"bakong_rate\"\n                                                    placeholder=\"\">\n                                                <span class=\"input-group-text\">\n                                                    ";
        ...
        echo $CMSNT->site("bakong_min");
        echo "\" name=\"bakong_min\">\n                                        </div>\n                                    </div>\n                                    <div class=\"row mb-4\">\n                                        <label class=\"col-sm-4 col-form-label\" for=\"example-hf-email\">Max</label>\n                                        <div class=\"col-sm-8\">\n                                            <input type=\"number\" class=\"form-control\"\n                                                value=\"";
        echo $CMSNT->site("bakong_max");
        echo "\" name=\"bakong_max\">\n                                        </div>\n                                    </div>\n                                </div>\n                                <div class=\"col-lg-12 col-xl-12\">\n                                    <div class=\"row mb-4\">\n                                        <label class=\"col-sm-6 col-form-label\"\n                                            for=\"example-hf-email\">";
        echo __("Note");
        echo "</label>\n                                        <div class=\"col-sm-12\">\n                                            <textarea id=\"bakong_notice\"\n                                                name=\"bakong_notice\">";
        echo $CMSNT->site("bakong_notice");
        echo "</textarea>\n                                        </div>\n                                    </div>\n                                </div>\n                                ";
    }
    ...
    echo format_currency($CMSNT->get_row("SELECT SUM(price) FROM payment_bakong WHERE `status` = 1 AND MONTH(created_at) = '" . $currentMonth . "' AND YEAR(created_at) = '" . $currentYear . "' ")["SUM(price)"]);
    echo "                                        </p>\n                                        <p class=\"mb-0 text-muted\">Tháng ";
    echo date("m");
    echo "</p>\n                                    </div>\n                                    <div class=\"ms-2\">\n                                        <span class=\"avatar text-bg-info rounded-circle fs-20\"><i\n                                                class='bx bxs-wallet-alt'></i></span>\n                                    </div>\n                                </div>\n                            </div>\n                        </div>\n                    </div>\n                    <div class=\"col-xl-6\">\n                        <div class=\"card custom-card\">\n                            <div class=\"card-body\">\n                                <div class=\"d-flex align-items-center\">\n                                    <div class=\"flex-fill\">\n                                        <p class=\"mb-1 fs-5 fw-semibold text-default\">\n                                            ";
    echo format_currency($CMSNT->get_row("SELECT SUM(price) FROM payment_bakong WHERE `status` = 1 AND YEAR(created_at) = " . $currentYear . " AND WEEK(created_at, 1) = " . $currentWeek . " ")["SUM(price)"]);
    ...
    foreach ($listDatatable as $row) {
        ...
        echo $row["trans_id"];
        echo "</b></td>\n                                        <td class=\"text-right\"><b style=\"color:blue;\">\$";
    ...
    echo __("Paid:");
    echo "                                                <strong\n                                                    style=\"color:red;\">";
    echo format_currency($CMSNT->get_row(" SELECT SUM(`price`) FROM `payment_bakong` WHERE " . $where . " AND `status` = 1 ")["SUM(`price`)"]);
