...
if(isset($_POST["RemoveAccounts"])) {
    ...
    foreach ($list as $account) {
        list($uid_remove) = explode("|", $account);
        $isRemove = $CMSNT->remove("product_stock", " `uid` = '" . $uid_remove . "' ");
        if($isRemove) {
            $value_remove++;
            $Mobile_Detect = new Mobile_Detect();
            $CMSNT->insert("logs", ["user_id" => $getUser["id"], "ip" => myip(), "device" => $Mobile_Detect->getUserAgent(), "createdate" => gettime(), "action" => "Xóa tài khoản (" . $uid_remove . ") khỏi kho hàng đang bán"]);
        }
    }
    ...
    $Mobile_Detect = new Mobile_Detect();
    $CMSNT->insert("logs", ["user_id" => $getUser["id"], "ip" => myip(), "device" => $Mobile_Detect->getUserAgent(), "createdate" => gettime(), "action" => "Import " . $value_add . " tài khoản vào kho hàng " . $code]);
    $my_text = $CMSNT->site("noti_action");
    $my_text = str_replace("{domain}", $_SERVER["SERVER_NAME"], $my_text);
    $my_text = str_replace("{username}", $getUser["username"], $my_text);
