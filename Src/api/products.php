...
foreach ($CMSNT->get_list(" SELECT * FROM `categories` WHERE `parent_id` != 0 AND `status` = 1 ") as $category) {
    $data_product = [];
    foreach ($CMSNT->get_list(" SELECT * FROM `products` WHERE `category_id` = '" . $category["id"] . "' AND `status` = 1 AND `allow_api` = 1 ") as $product) {
        $stock = $product["supplier_id"] != 0 ? $product["api_stock"] : getStock($product["code"]);
        $data_product[] = ["id" => $product["id"], "name" => $product["name"], "price" => $product["price"], "amount" => (int) $stock, "description" => $product["short_desc"], "flag" => $product["flag"], "min" => $product["min"], "max" => $product["max"]];
    }
